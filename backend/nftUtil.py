...
def SendEmail(arguments):
    ...
    emailText = _ConcatenateToString(("From: ", _GMAIL_USER, "\n", "To: ", toAddresses, "\n", ("CC: " + ccAddresses + "\n" if ccAddresses else ""), "Subject: ", subject[:255], "\n", body))
...
def SetTokensMinted(tokensMinted):
    ...
    _DB[_COLLECTION_STATISTICS].update_one({"_id": {"$ne": _EMPTY_STRING}}, {"$set": {FIELD_TOKENS_MINTED: tokensMinted}})  # A dummy filter that is required to get the first and only document.
...
def SetTotalTokens(totalTokens):
    ...
    _DB[_COLLECTION_STATISTICS].update_one({"_id": {"$ne": _EMPTY_STRING}}, {"$set": {FIELD_TOTAL_TOKENS: totalTokens}})  # A dummy filter that is required to get the first and only document.
...
def CreateMetadata(nftDefinition):
    ...
    for i, doc in enumerate(_DB[_COLLECTION_NFTS].find()):
        ...
        _DB[_COLLECTION_NFTS].update_one({FIELD_ID: doc[FIELD_ID]}, {"$set": {FIELD_TOKEN_IS_EXPOSABLE: tokenIsExposable, FIELD_EXTERNAL_URL: externalUrl, FIELD_IMAGE_URI: imageUri}})
...
def GetDocMetadata(doc):
    ...
    """
    metadata = {
        "description": doc[FIELD_COLLECTION_NAME] + " token number " + str(doc[FIELD_ID]) + ("" if doc[FIELD_RARITY] is None else ", with a rarity of '" + doc[FIELD_RARITY] + "'"),
...
def _LogLine(message, level, tokenId=_EMPTY_STRING, secondaryMessage=_EMPTY_STRING, sendEmail=True):
    ...
    if level == _LEVEL_ERROR and sendEmail:
        emailBody = _ConcatenateToString(("Hey Winnie and Wilson,\n\n", message, "\n\n", (secondaryMessage + "\n\n" if secondaryMessage else _EMPTY_STRING), "Hope you catch a lizard,\nBoo\n"))
